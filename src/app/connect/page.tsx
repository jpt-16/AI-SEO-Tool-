...
export default async function ConnectPage({ searchParams }: PageProps<"/connect">) {
  ...
  return (
    <div className="grid min-h-screen grid-cols-1 lg:grid-cols-2">
      <section className="rule-right relative flex flex-col justify-between gap-16 overflow-hidden px-8 py-12 lg:px-19 lg:py-16">
        <svg
          aria-hidden="true"
          viewBox="0 0 720 900"
        ...
        <div className="relative flex max-w-[540px] flex-col gap-5.5">
          ...
          <h1 className="text-[clamp(38px,4.2vw,56px)] leading-[1.06] font-medium tracking-[-0.01em] text-pretty">
      ...
      <section className="flex flex-col justify-center gap-9 px-8 py-12 lg:px-23 lg:py-16">
        <div className="flex flex-col gap-3">
          <span className="eyebrow">Step 1 of 2</span>
          <h2 className="text-[34px] font-medium tracking-[-0.01em]">Connect Google Search Console</h2>
        </div>
        ...
        <ol className="panel flex flex-col overflow-hidden">
          {STEPS.map((step, i) => (
            <li key={step.title} className="flex gap-5 border-line px-6 py-5.5 [&+&]:border-t">
